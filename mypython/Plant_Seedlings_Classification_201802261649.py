...
new_width,new_high=64,64
train_data_path='D:\\Desktop\\kaggle\\Plant_Seedlings_Classification\\train_1'
test_data_path='D:\\Desktop\\kaggle\\Plant_Seedlings_Classification\\test_1'
...
while load_test_data==False:
    ...
    np.save('Plant_Seedlings_Classification\\plant_new_data_test_1',np.array(faces_kt))
    print(np.array(faces_kt).shape)                  
    print('step2')
    load_test_data=True
x_test_sum=np.load('Plant_Seedlings_Classification\\plant_new_data_test_1.npy')
...
#             print(path+file)
            ...
            images=images[:,:,:3]
            faces_kt.append(images)
        np.save('Plant_Seedlings_Classification\\plant_new_data_2'+str(i),np.array(faces_kt))
        print(np.array(faces_kt).shape)                 
    print('step4')
    load_data=True
...
for i in range(12):
    images=np.load('Plant_Seedlings_Classification\\plant_new_data_2%s.npy'%(i))
...
def train_(x_train,x_test,y_train,y_test):
    ...
    layer6=tf.layers.conv2d(layer5, filters_*16, 3, 1,'same',name='layer6')
...
#     layer8=tf.layers.max_pooling2d(layer8, 2, 2,'same')
    ...
    layer9=tf.reshape(layer5, [-1,2*2*filters_*16])
    
    layer10=tf.layers.dense(layer9, 4096)
    ...
    layer11=tf.layers.dense(layer10, 2048)
        ...
        for i in range(101):
            ...
            for j in range(n_batch):
                ...
                for m in range(1,128):
                    feed_data_x=np.vstack((feed_data_x,np.reshape(x_train[m*36+j],[1,new_width,new_high,3])))
                    feed_data_y=np.vstack((feed_data_y,np.reshape(y_train[m*36+j],[1,12])))
...
#                 print(a.shape,b.shape,c.shape,d.shape)
            pred1,acc1=sess.run([prediction,acc],{x:x_test,y:y_test,keep_prob_:1.0})
