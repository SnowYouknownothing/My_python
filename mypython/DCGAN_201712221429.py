...
#     G_layer2=tf.layers.conv2d_transpose(G_layer1, filters=1024, kernel_size=2, strides=2, padding='same', activation=tf.nn.relu)
#     G_layer2=tf.nn.dropout(G_layer2,keep_prob=0.5) 
    
    G_layer3=tf.layers.conv2d_transpose(G_layer1, filters=512, kernel_size=4, strides=1, padding='valid', activation=tf.nn.relu)
    G_layer3=tf.nn.dropout(G_layer3,keep_prob=0.5)
    
    G_layer4=tf.layers.conv2d_transpose(G_layer3, filters=256, kernel_size=3, strides=2, padding='same', activation=tf.nn.relu)
...
#     f4=tf.nn.dropout(f3,keep_prob=0.5)  
    ...
    conv2=tf.layers.conv2d(layer1, filters=128, kernel_size=3, strides=1, padding='same',activation=tf.nn.relu,name='2')
...
#     ff4=tf.nn.dropout(ff3,keep_prob=0.5)  
    ...
    conv2_f=tf.layers.conv2d(layer1_f, filters=128, kernel_size=3, strides=1, padding='same',activation=tf.nn.relu,name='2',reuse=True)
    pool2_f=tf.layers.max_pooling2d(conv2_f, pool_size=2, strides=2, padding='same')
    ...
    pool3_f=tf.layers.max_pooling2d(conv3_f, pool_size=2, strides=2, padding='valid')
    ...
    pool4_f=tf.layers.max_pooling2d(conv4_f, pool_size=2, strides=2, padding='same')
