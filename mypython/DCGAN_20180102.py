...
#     G_layer2=tf.nn.dropout(G_layer2,keep_prob=0.5) 
    ...
    G_layer3=tf.layers.conv2d_transpose(G_layer1, filters=1024, kernel_size=4, strides=1, padding='valid')
    G_layer3=tf.layers.batch_normalization(G_layer3)
    G_layer3=tf.nn.relu(G_layer3)
    G_layer3=tf.nn.dropout(G_layer3,keep_prob=0.8)
    
    G_layer4=tf.layers.conv2d_transpose(G_layer3, filters=512, kernel_size=3, strides=2, padding='same')
...
#     f4=tf.nn.dropout(f3,keep_prob=0.5)  
    conv1=tf.layers.conv2d(inputs_real, filters=64, kernel_size=3, strides=2, padding='same',activation=tf.nn.relu,name='1')
#     pool1=tf.layers.max_pooling2d(conv1, pool_size=2, strides=2, padding='same')
    layer1=tf.nn.dropout(conv1, keep_prob=0.8)
    
    conv2=tf.layers.conv2d(layer1, filters=128, kernel_size=3, strides=2, padding='same',name='2')
    pool2=conv2
    pool2=tf.maximum(0.01*pool2,pool2)
#     pool2=tf.layers.max_pooling2d(pool2, pool_size=2, strides=2, padding='same')
    layer2=tf.nn.dropout(pool2, keep_prob=0.8)

    conv3=tf.layers.conv2d(layer2, filters=256, kernel_size=3, strides=2, padding='same',name='3')
    pool3=conv3
    pool3=tf.maximum(0.01*pool3,pool3)
#     pool3=tf.layers.max_pooling2d(pool3, pool_size=2, strides=2, padding='valid')
    layer3=tf.nn.dropout(pool3, keep_prob=0.8)   

    conv4=tf.layers.conv2d(layer3, filters=512, kernel_size=3, strides=2, padding='same',name='4')
    pool4=conv4
    pool4=tf.maximum(0.01*pool4,pool4)    
#     pool4=tf.layers.max_pooling2d(pool4, pool_size=2, strides=2, padding='same')
    layer4=tf.nn.dropout(pool4, keep_prob=0.8)
...
#     ff4=tf.nn.dropout(ff3,keep_prob=0.5)  
    conv1_f=tf.layers.conv2d(G_outputs, filters=64, kernel_size=3, strides=2, padding='same',activation=tf.nn.relu,name='1',reuse=True)
#     pool1_f=tf.layers.max_pooling2d(conv1_f, pool_size=2, strides=2, padding='same')
    layer1_f=tf.nn.dropout(conv1_f, keep_prob=0.8)
    
    conv2_f=tf.layers.conv2d(layer1_f, filters=128, kernel_size=3, strides=2, padding='same',name='2',reuse=True)
    pool2=conv2_f
    pool2=tf.maximum(0.01*pool2,pool2)   
#     pool2_f=tf.layers.max_pooling2d(pool2, pool_size=2, strides=2, padding='same')
    layer2_f=tf.nn.dropout(pool2, keep_prob=0.8)

    conv3_f=tf.layers.conv2d(layer2_f, filters=256, kernel_size=3, strides=2, padding='same',name='3',reuse=True)
    pool3=conv3_f
    pool3=tf.maximum(0.01*pool3,pool3)     
#     pool3_f=tf.layers.max_pooling2d(pool3, pool_size=2, strides=2, padding='valid')
    layer3_f=tf.nn.dropout(pool3, keep_prob=0.8)   

    conv4_f=tf.layers.conv2d(layer3_f, filters=512, kernel_size=3, strides=2, padding='same',name='4',reuse=True)
    pool4=conv4_f
    pool4=tf.maximum(0.01*pool4,pool4)     
#     pool4_f=tf.layers.max_pooling2d(pool4, pool_size=2, strides=2, padding='same')
    layer4_f=tf.nn.dropout(pool4, keep_prob=0.8)
...
list_g_loss,list_d_loss,list_g,list_o_d_loss_real,list_o_d_loss_fake=[],[],[],[],[]
j=0
...
#         sess.run(tf.assign(lr,0.001*(0.5**epoch)))
        for i in range(550):
            ...
            list_o_d_loss_real.append(o_d_loss_real)
            list_o_d_loss_fake.append(o_d_loss_fake)
            j+=1 
            if j%165==0:
...
# 显示结果
plt.figure()
plt.plot(list_d_loss,'r')
plt.plot(list_g_loss,'g')
plt.plot(list_o_d_loss_real,'b')
plt.plot(list_o_d_loss_fake,'y')
plt.show()
