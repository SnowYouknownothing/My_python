...
def w_b(x):
    ...
    for l in range(len(x)):
        w.append(2*random.random((x[i],x[i+1]))-1)
        b.append(2*random.random((x[i+1],1)-1))
    return w,b
...
# #            y_o，（1,1）         
#         # 正向传播结束
#         error_o=sigmord_deriv(l_o)*(y_output_suiji-y_o)  
# #         error_o(1,1)
# #         error_j=sigmord_j*(1-sigmord_j)*error_o*(w_jo.T)
#         error_j=sigmord_deriv(l_j)*error_o*(w_jo.T)
# #         error_j(1,2)      
#         error_k=sigmord_deriv(l_k) * dot(error_j,(w_kj.T))
# #         error_k(1,3)      
#         error_l=sigmord_deriv(l_l) * dot(error_k,(w_lk.T))
# #         error_k(1,3)
#         error_m=sigmord_deriv(l_m) * dot(error_l,(w_ml.T))
#         
...
for i in range(1):
    ...
    w_im,w_ml,w_lk,w_kj,w_jo,b_m,b_l,b_k,b_j,b_o = PD(learning_rate,epochs,daying,x_input,y_output,index_hang,index_lie,layel)
    zhunquelv,i_x1=[],[]
