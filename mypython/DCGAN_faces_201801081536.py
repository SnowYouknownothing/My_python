...
def Gen(inputs):
    with tf.variable_scope('G'):
        ...
        G_layer5=tf.maximum(0.01*G_layer5,G_layer5)
        ...
        G_layer6=tf.maximum(0.01*G_layer6,G_layer6)
...
def Dis(inputs,reuse_=False):
    with tf.variable_scope('D'):
        ...
        layer_4=tf.reshape(layer_4, [-1,3*3*filters_d*8])
        
        full_conect_1=tf.layers.dense(layer_4,3*3*filters_d*8,name='5',reuse=reuse_)
...
#         
        logits=tf.layers.dense(full_conect_1,1,name='6',reuse=reuse_)
