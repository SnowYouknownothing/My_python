...
new_width,new_high=32,32
train_data_path='D:\\Desktop\\kaggle\\Plant_Seedlings_Classification\\train'
test_data_path='D:\\Desktop\\kaggle\\Plant_Seedlings_Classification\\test'
...
while load_test_data==False:
    ...
    np.save('Plant_Seedlings_Classification\\plant_new_data_test_0',np.array(faces_kt))
    print(np.array(faces_kt).shape)                  
    print('step2')
    load_test_data=True
x_test_sum=np.load('Plant_Seedlings_Classification\\plant_new_data_test_0.npy')
...
#             print(path+file)
            ...
            images=images[:,:,:3]
            faces_kt.append(images)
        np.save('Plant_Seedlings_Classification\\plant_new_data_1'+str(i),np.array(faces_kt))
        print(np.array(faces_kt).shape)                 
    print('step4')
    load_data=True
...
for i in range(12):
    images=np.load('Plant_Seedlings_Classification\\plant_new_data_1%s.npy'%(i))
...
def train_(x_train,x_test,y_train,y_test):
    ...
    layer5=tf.layers.conv2d(layer4, filters_*8, 3, 1,'same',name='layer5')
    ...
    layer6=tf.layers.conv2d(layer5, filters_*8, 3, 1,'same',name='layer6')
...
#     layer8=tf.layers.max_pooling2d(layer8, 2, 2,'same')
    ...
    layer9=tf.reshape(layer4, [-1,2*2*filters_*8])
...
#     layer11=tf.nn.dropout(layer11, keep_prob_)
    ...
    output=tf.layers.dense(layer10,12,tf.nn.softmax)
    ...
    with tf.Session() as sess:
        ...
        for i in range(31):
            ...
            for j in range(n_batch):
                feed_data_x=np.reshape(x_train[j],[1,32,32,3])
                feed_data_y=np.reshape(y_train[j],[1,12])
                for m in range(1,64):
                    feed_data_x=np.vstack((feed_data_x,np.reshape(x_train[m*72+j],[1,32,32,3])))
